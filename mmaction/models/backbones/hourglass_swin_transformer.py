...
class TokenClusteringBlock(nn.Module):
    def __init__(self, clustering_shape, n_iters=5, temperature=0.01, window_size=3):
        super().__init__()
    ...
    def calc_init_centroid(self, images):
        """
        ...
        """
        batchsize, channels, d, height, width = images.shape
        device = images.device

        centroids = F.adaptive_avg_pool3d(
            images, self.clustering_shape,
        )
        num_spixels = reduce(mul, self.clustering_shape)

        with torch.no_grad():
            # num_spixels = num_spixels_width * num_spixels_height
            labels = (
                torch.arange(num_spixels, device=device)
                .reshape(1, 1, *centroids.shape[-3:])
                .type_as(centroids)
            )
            init_label_map = F.interpolate(
                labels, size=(d, height, width), mode="trilinear"
            ).type_as(centroids)
            init_label_map = init_label_map.repeat(batchsize, 1, 1, 1, 1)
    ...
    def forward(self, pixel_features):
        # pixel_features: (B, D, H, W, C)
        # num_spixels_height, num_spixels_width = num_spixels
        # num_spixels = num_spixels_width * num_spixels_height
        B, D, H, W, C = pixel_features.shape
        pixel_features = pixel_features.permute(0, 4, 1, 2, 3)  # (B, C, D, H, W)
        num_spixels = reduce(mul, self.clustering_shape)
        ...
        relative_labels_widths = init_label_map[:, None] % self.clustering_shape[-1] - spixels_number % self.clustering_shape[-1]
        relative_labels_heights = torch.div(init_label_map[:, None], self.clustering_shape[-1], rounding_mode='trunc') % self.clustering_shape[1] - torch.div(spixels_number, self.clustering_shape[-1], rounding_mode='trunc') % self.clustering_shape[1]
        relative_labels_d = torch.div(init_label_map[:, None], self.clustering_shape[-1] * self.clustering_shape[1], rounding_mode='trunc') - torch.div(spixels_number, self.clustering_shape[-1] * self.clustering_shape[1], rounding_mode='trunc')
        mask = torch.logical_and(torch.logical_and(torch.abs(relative_labels_widths) <= self.r, torch.abs(relative_labels_heights) <= self.r), torch.abs(relative_labels_d) <= self.r)
        mask_dist = (~mask) * 1e16
        ...
        for _ in range(self.n_iters):
            ...
            affinity_matrix = (-dist_matrix * self.temperature).softmax(1)
        ...
        return spixel_features.permute(0, 2, 1).reshape(B, *self.clustering_shape, -1), hard_labels, self.clustering_shape
    ...
    def extra_repr(self):
        return f"clustering_shape={self.clustering_shape}, n_iters={self.n_iters}"
...
class TokenReconstructionBlock(UnpoolingBase):
    def __init__(self, k=25, temperature=0.01):
        super().__init__()
    ...
    def _forward(self, x, state: State):
        ...
        ds[ds < 0] = 0
        weight = torch.exp(-self.temperature * ds)
        if self.k >= 0:
...
class WindowAttention3D(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., ckpt_window_size=(2, 12, 12)):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * ckpt_window_size[0] - 1) * (2 * ckpt_window_size[1] - 1) * (2 * ckpt_window_size[2] - 1), num_heads))  # 2*Wd-1 * 2*Wh-1 * 2*Ww-1, nH
    ...
    def resize_rpb_table(self, relative_position_bias_table):
        table = relative_position_bias_table.permute(1, 0).reshape(
            -1, 2 * self.ckpt_window_size[0] - 1, 2 * self.ckpt_window_size[1] - 1, 2 * self.ckpt_window_size[2] - 1
        )
        table = F.interpolate(
            table[None],
            (2 * self.window_size[0] - 1, 2 * self.window_size[1] - 1, 2 * self.window_size[1] - 1),
            mode="trilinear",
        )
        table = table.unsqueeze(0).view(
            -1, (2 * self.window_size[0] - 1) * (2 * self.window_size[1] - 1) * (2 * self.window_size[1] - 1)
        )
    ...
    def forward(self, x, mask=None):
        ...
        if (
            ...
            or self.ckpt_window_size[2] != self.window_size[2]
        ):
            self.relative_position_bias_table = nn.Parameter(
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2,7,7), shift_size=(0,0,0),
        ...
        self.attn = WindowAttention3D(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, ckpt_window_size=ckpt_window_size)
...
class HourglassBasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        clustering_shape = token_clustering_block.clustering_shape
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock3D(
                dim=dim,
                num_heads=num_heads,
                window_size=window_size if i < clustering_location else clustering_shape,
                shift_size=(0,0,0) if (i % 2 == 0) else (tuple(j // 2 for j in (window_size if i < clustering_location else clustering_shape))),
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=drop,
    ...
    def get_mask(self, x, window_size, shift_size):
        B, C, D, H, W = x.shape
    ...
    def cluster(self, x, reconstructer):
        ...
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
        _, Dp, Hp, Wp, _ = x.shape
        x = window_partition(x, self.window_size)  # B*H*W, N, C

        out, hard_labels, shape = self.token_clustering_block(x.view(-1, *self.window_size, C))  # B*H*W, Wh*Ww, C
        # reconstructer.update_state(hard_labels=hard_labels)
        reconstructer.update_state(
            feat_before_pooling=x.view(x.shape[0], -1, C)
        )
        reconstructer.update_state(feat_after_pooling=out.view(out.shape[0], -1, C))

        # merge window
        # out = out.view(-1, *shape, C)
        Dp = Dp // self.window_size[0] * shape[0]
        Hp = Hp // self.window_size[1] * shape[1]
        Wp = Wp // self.window_size[2] * shape[2]
        out = window_reverse(out, shape, B, Dp, Hp, Wp)  # B, d, h, w, C
        return out, shape

    def reconstruct(self, x, reconstructer, ori_shape):
        B, D, H, W, C = x.shape
        x = window_partition(x, self.window_size)
        x = x.view(x.shape[0], -1, C)

        x, _ = reconstructer.call(x)
        self.window_size = self.org_window_size

        # merge windows
        x = x.view(x.shape[0], *self.window_size, C)
        D, H, W = ori_shape
        ...
        x = window_reverse(x, self.window_size, B, Dp, Hp, Wp)  # B H' W' C
        if D != Dp or H != Hp or W != Wp:
            x = x[:, :D, :H, :W, :].contiguous()
        return x
    ...
    def forward(self, x):
        ...
        B, C, D, H, W = x.shape
        attn_mask = self.get_mask(x, self.window_size, self.shift_size)
        # window_size, shift_size = get_window_size((D,H,W), self.window_size, self.shift_size)
        ...
        for i, blk in enumerate(self.blocks):
            if i == self.clustering_location:
                x, clustering_shape = self.cluster(x, reconstructer)
                self.org_window_size = self.window_size
                self.org_shift_size = self.shift_size
                self.window_size = clustering_shape
                self.shift_size = tuple(i // 2 for i in clustering_shape)
                clustered = True
                attn_mask = self.get_mask(x.permute(0, 4, 1, 2, 3), self.window_size, self.shift_size)
            x = blk(x, attn_mask)
        # x = x.view(B, -1, H, W, C)

        if clustered:
            x = self.reconstruct(x, reconstructer, (D, H, W))
            self.window_size = self.org_window_size
            self.shift_size = self.org_shift_size
...
class HourglassSwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        print("token_clustering_block: ", token_clustering_cfg)
        print("token_reconstruction_block: ", token_reconstruction_cfg)
        ...
        for i_layer in range(self.num_layers):
            if clustering_location < depths[i_layer] and clustering_location >= 0:
                layer = HourglassBasicLayer(
                ...
                layer = BasicLayer(
                    ...
                    downsample=PatchMerging if i_layer<self.num_layers-1 else None,
                    use_checkpoint=use_checkpoint,)
            clustering_location -= depths[i_layer]
    ...
    def train(self, mode=True):
        ...
        super(HourglassSwinTransformer3D, self).train(mode)
        self._freeze_stages()
